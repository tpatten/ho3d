...
if __name__ == '__main__':
    ...
    while(True):
        ...
        if args['visType'] == 'open3d':
            ...
            for uv in np.asarray(hand_uv):
                img[int(uv[1]), 640 - int(uv[0])] = [255, 0, 0]
            ...
            for i, m in enumerate(mList):
                ...
                if colorList[i] == 'r':
                    mesh.vertex_colors = o3d.utility.Vector3dVector(
                        np.tile(np.array([[0.6, 0.2, 0.2]]), [numVert, 1]))
                elif colorList[i] == 'g':
                    mesh.vertex_colors = o3d.utility.Vector3dVector(
                        np.tile(np.array([[0.5, 0.5, 0.5]]), [numVert, 1]))
                else:
